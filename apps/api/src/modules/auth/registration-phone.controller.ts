...
function providerError(error: PhoneOtpProviderError): Response {
  if (error.reason === 'not_configured')
    return Response.json({ message: 'ยังไม่ได้ตั้งค่า BoostSMS API Key', field: 'phone_number' }, { status: 503 })
  if (error.providerStatus === 402)
    return Response.json({ message: 'เครดิต SMS OTP ไม่เพียงพอ', field: 'phone_number' }, { status: 402 })
  if (error.rateLimitMessage)
    return Response.json(
      { message: error.rateLimitMessage, field: 'phone_number' },
      { status: 429 },
    )
  if (error.providerStatus === 401 || error.providerStatus === 403)
    return Response.json({ message: 'การตั้งค่า BoostSMS API Key ไม่ถูกต้อง', field: 'phone_number' }, { status: 503 })
  console.error('Unable to send registration phone OTP', error)
  if (error.providerStatus === 400)
    return Response.json(
      { message: 'BoostSMS ปฏิเสธคำขอ OTP กรุณาตรวจสอบเบอร์มือถือหรือติดต่อผู้ดูแลระบบ', field: 'phone_number' },
      { status: 400 },
    )
  return Response.json({ message: 'ระบบ OTP ยังไม่พร้อมใช้งาน กรุณาลองใหม่อีกครั้ง' }, { status: 503 })
}

async function requestRegistrationPhone(
  body: PhoneRequest,
  verifyTurnstile: (token: string | undefined) => Promise<boolean>,
) {
  if (!(await verifyTurnstile(body.turnstile_token))) {
    return Response.json(
      { message: 'กรุณายืนยัน Cloudflare Turnstile ก่อนขอรหัส OTP', field: 'turnstile_token' },
      { status: 400 },
    )
  }
  try {
    ...
    if (error === 'phone_in_use')
      return Response.json({ message: 'เบอร์มือถือถูกใช้งานกับบัญชีอื่นแล้ว', field: 'phone_number' }, { status: 409 })
...
export async function verifyRegistrationPhoneResponse(body: PhoneVerify) {
  try {
    ...
    if (result === 'phone_in_use')
      return Response.json({ message: 'เบอร์มือถือถูกใช้งานกับบัญชีอื่นแล้ว', field: 'phone_number' }, { status: 409 })
    if (result === 'expired')
      return Response.json({ message: 'รหัส OTP หมดอายุแล้ว กรุณาขอรหัสใหม่', field: 'otp' }, { status: 410 })
    if (result === 'invalid_otp')
      return Response.json({ message: 'รหัส OTP ไม่ถูกต้อง', field: 'otp' }, { status: 400 })
