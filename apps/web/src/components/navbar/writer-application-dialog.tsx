...
export function WriterApplicationDialog({ open, onOpenChange }: Props) {
  ...
  useEffect(() => {
    ...
    void getBankConfigs(accessToken).then((result) => setBanks(result.banks)).catch(() => toast.error('ไม่สามารถโหลดรายการธนาคารได้')).finally(() => setIsLoadingBanks(false))
  }, [accessToken, open])
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="gap-5 rounded-2xl p-5 sm:max-w-md sm:p-6">
        <DialogHeader>
          ...
          <DialogDescription>กรอกข้อมูลบัญชีธนาคารสำหรับรับรายได้จากผลงาน ข้อมูลนี้ใช้เพื่อการโอนเงินจากระบบเท่านั้น ระบบจะแจ้งผลภายใน 1–3 วัน</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2"><Label htmlFor="writer-bank-code">ธนาคาร</Label><Select required value={form.bank_code} onValueChange={(value) => setForm({ ...form, bank_code: value })} disabled={isLoadingBanks}><SelectTrigger id="writer-bank-code" className="w-full"><SelectValue placeholder={isLoadingBanks ? 'กำลังโหลดรายการธนาคาร...' : 'เลือกธนาคาร'} /></SelectTrigger><SelectContent position="popper" className="w-full">{banks.map((bank) => <SelectItem key={bank.code} value={bank.code}><span className="flex items-center gap-2"><img src={`${SITE_CONFIG.apiUrl}/writer/${bank.logo}`} alt="" className="size-5 object-contain" />{bank.name}</span></SelectItem>)}</SelectContent></Select></div>
